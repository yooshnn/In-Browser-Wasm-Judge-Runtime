...
// browser runtime 기준 compile 결과는
// execution worker로 직접 넘길 수 있는 serializable payload를 사용한다.
...
### 왜 `ExecutableArtifact`를 opaque id로 두지 않는가

browser runtime에서 testcase별 execution worker 격리를 하려면 compile 산출물을 worker-local handle로 두기보다
직렬화 가능한 payload로 유지하는 편이 낫다.

이 구조는 다음을 쉽게 만든다.

- testcase마다 독립 worker 실행
- timeout 시 worker terminate 기반 TLE 처리
- 메모리/출력 제한 실패 후 다음 testcase로의 빠른 복구
